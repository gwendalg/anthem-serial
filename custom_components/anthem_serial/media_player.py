...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    anthem = AnthemEntity(config.get(CONF_NAME), config.get(CONF_PORT))
    add_entities([anthem])
    return True
...
class AnthemEntity(MediaPlayerEntity):
    ...
    def __init__(self, name, port):
        ...
        try:
            ser = serial.serial_for_url(
                self._port, baudrate=115200, xonxoff=True, timeout=2, writeTimeout=1
            )
        except serial.SerialException as e:
            _LOGGER.error("Unable to open serial port %s to %s: %s" % (port, name, e))
            raise e
        self._ser_io = io.TextIOWrapper(
            io.BufferedRWPair(ser, ser), line_buffering=True, newline="\n"
        )
    ...
    def update(self):
        ...
        m = re.match(
            r".*P1S(?P<source>\d{1})V(?P<volume>-*\d{2})M(?P<mute>\d{1}).*",
            raw_state,
            re.DOTALL,
        )
        if m:
